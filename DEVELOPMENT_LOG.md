...
## 階段 19：架構圖功能上線後的一連串使用者回饋 —— 排版、上傳整合、宣告頁面整併

這個階段從使用者要求「上傳資料後自動畫構面路徑架構圖」開始，中間陸續收到好幾輪真實使用回饋，最後收斂成把「宣告」頁面整個併進「上傳」頁面。照時間順序記錄：

**架構圖第一版排版問題**：第一次實測 10 個構面的真實資料，圖高達 601×2891px，因為每個構面底下的題項各自畫成獨立節點＋連線，dot 的 cluster layout 把每個構面的題項群組往上堆疊。改成把題項直接列在構面節點的 label 裡（一個構面一個節點，不再是一個節點+多個子節點），大幅壓縮版面（見 commit `9d4b80e`）。

**「宣告」頁面完全沒接圖表**：使用者反應在「宣告」頁面看不到架構圖，一開始以為是 L3 即時預覽的 bug，實際用同樣參數直接測 `/diagram` 後端完全正常——真正原因是 Phase 5b 加 `/diagram` 端點時，只接了「上傳」頁面跟「測量/結構診斷」頁面，完全漏掉「宣告」（L0）頁面（見 commit `1f478fd`）。這提醒了一件事：新增一個橫跨多頁面的功能時，要把「這個功能理論上該出現在哪些頁面」列清楚，不能只挑幾個順手的頁面接。

**結構路徑改用檔案讀取**：使用者不想每次都手動打字宣告結構路徑，問「可以直接讀資料檔嗎」。這裡有一個方法論上的界線要先講清楚：構面（測量模型）可以從欄位名稱自動偵測，但結構路徑是研究者的理論假設，沒辦法從問卷回答的數字反推——所以做法是讓 `.xlsx` 檔案可以額外帶一個叫 `structural_model` 的工作表（`dependent`／`independent` 兩欄，一列一組），`load_data()` 偵測到就自動讀取，這是唯一一種「自動讀取」的結構路徑輸入，而且只因為是研究者自己寫進第二個工作表、不是從資料反推出來的（見 commit `e8a5574`）。

**上傳即宣告**：使用者接著問「這樣不就可以在上傳的時候順便完成宣告」——的確可以，因為宣告只需要「有測量模型 + 有結構模型」，兩者上傳當下就都有了。`/upload` 現在會在這個 session 還沒有任何宣告、且這次上傳的 Excel 帶有 `structural_model` 工作表時，自動呼叫跟手動宣告同一套 `audit_db.create_declaration()`，真正寫進資料庫、蓋時間戳記，不是介面裝飾（見 commit `5123f4d`）。如果這個 session 已經宣告過，不會被覆蓋。

**拿掉「宣告」頁面**：確認自動宣告機制運作正常後，使用者要求把「宣告」頁面整個刪掉，只保留「上傳」頁面。刪除前先確認了一件事：如果只上傳 CSV，或 Excel 沒有附 `structural_model` 工作表，使用者會完全沒有地方可以宣告——這是拿掉整個頁面必須要接受的代價，跟使用者確認過。實作上，「上傳」頁面現在多了一個選填的結構路徑 `.txt` 上傳欄位（沿用「宣告」頁面原本就有的同一套格式跟 `parse_line_dict` 解析邏輯），上傳主資料檔沒有帶結構路徑時，如果有附這個 `.txt`，前端會直接呼叫 `/declare`（原本「宣告」頁面按鈕做的事，後端完全沒改），一樣寫進宣告記錄。等於把「宣告」從一個獨立步驟，變成「上傳」流程裡的一個分支，而不是拿掉這個功能本身。

**驗證方式**：模擬三種情境都測過——(1) Excel 帶 `structural_model` 工作表：上傳即自動宣告；(2) CSV + 另外上傳 `.txt`：上傳頁面呼叫 `/declare` 補上宣告；(3) 純 CSV 沒有任何結構路徑：只顯示構面圖，不宣告，跟使用者說明過這是預期行為。125 個測試全過，後端完全沒改動（唯一動到後端邏輯的是階段中段的「上傳即宣告」那次，已經個別測過）。

## 階段 20：以既有統計與首次建議為基礎的 LLM 後續優化

使用者要求保留「上傳 → 統計分析 → 第一次優化 → 模擬」作為不可變基礎。因此本階段沒有修改 `app/stats_engine.py`、R bridge 或既有最佳化演算法。

新增 L6 討論層：`optimization_sessions` 凍結基準指標、模型宣告與第一次優化結果；`optimization_messages` 保存對話；`optimization_scenarios` 保存候選方案的參數與正式模擬結果。後續方案一律交由既有 `optimize_unified()` 計算，LLM 僅負責解釋與呼叫工具，不能自行產生統計數字。
